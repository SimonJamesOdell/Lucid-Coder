...
test('git save button stays disabled until settings change', async ({ page, request }) => {
  await ensureBootstrapped({ request })

  const projectName = `E2E Git Dirty State ${Date.now()}`
  ...
  await page.route('**/api/settings/git', async (route) => {
    if (route.request().method() === 'GET') {
      await route.fulfill({
        ...
        body: JSON.stringify({
          success: true,
          settings: { provider: 'github', defaultBranch: 'main', tokenPresent: true }
        })
      })
      return
    }
  ...
  await createProject(page, projectName, 'Git dirty state')

  await page.getByTestId('git-tab').click()
  await expect(page.getByTestId('git-tab-panel')).toBeVisible()

  await expect(page.getByTestId('git-save-preferences')).toBeDisabled()
  await page.getByTestId('project-default-branch').fill('release')
  await expect(page.getByTestId('git-save-preferences')).toBeEnabled()
