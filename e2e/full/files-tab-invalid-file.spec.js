...
test('files tab rejects invalid file names from context menu', async ({ page, request }) => {
  await ensureBootstrapped({ request })

  const projectName = `E2E Files Invalid File ${Date.now()}`
  ...
  await createProject(page, projectName, 'Files invalid file flow')
  ...
  expect(alertDialog.message()).toContain('Invalid file name.')
