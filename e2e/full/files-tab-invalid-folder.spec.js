...
test('files tab rejects invalid folder names from context menu', async ({ page, request }) => {
  await ensureBootstrapped({ request })

  const projectName = `E2E Files Invalid Folder ${Date.now()}`
  ...
  await createProject(page, projectName, 'Files invalid folder flow')

  await page.getByTestId('files-tab').click()
  await expect(page.getByTestId('file-tree')).toBeVisible()

  page.once('dialog', (dialog) => dialog.accept('bad/name'))
  ...
  expect(alertDialog.message()).toContain('Invalid folder name.')
