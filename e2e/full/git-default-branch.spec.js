...
test('git tab updates default branch and shows status message', async ({ page, request }) => {
  await ensureBootstrapped({ request })

  const projectName = `E2E Git Default Branch ${Date.now()}`
  ...
  await createProject(page, projectName, 'Git default branch update')
  ...
  await page.getByTestId('git-update-default-branch').click()

  await expect(page.getByText('Default branch set to release.')).toBeVisible()
