...
describe('autopilotFeatureRequest', () => {
  ...
  test('cancels immediately at start of guidance loop', async () => {
    ...
    const deps = {
      ...
      rollback: vi.fn(),
      consumeUserUpdates: vi.fn(() => []),
      shouldCancel: createStackTargetedShouldCancel(601),
      shouldPause: vi.fn(() => false),
      reportStatus: vi.fn(),
      getDiffForFiles: vi.fn().mockResolvedValue('diff ok'),
      appendEvent
  ...
  test('cancels during verification fix loop', async () => {
    ...
    const deps = {
      ...
      rollback: vi.fn(),
      consumeUserUpdates: vi.fn(() => []),
      shouldCancel: createStackTargetedShouldCancel(517),
      shouldPause: vi.fn(() => false),
      reportStatus: vi.fn(),
      getDiffForFiles: vi.fn().mockResolvedValue('diff ok'),
      appendEvent
  ...
  test('cancels after verification fix pause before retrying', async () => {
    ...
    const deps = {
      ...
      rollback: vi.fn(),
      consumeUserUpdates: vi.fn(() => []),
      shouldCancel: createStackTargetedShouldCancel(565),
      shouldPause: vi.fn(() => false),
      reportStatus: vi.fn(),
      getDiffForFiles: vi.fn().mockResolvedValue('diff ok'),
      appendEvent
  ...
  test('cancels while awaiting user guidance', async () => {
    ...
    const deps = {
      ...
      rollback: vi.fn(),
      consumeUserUpdates: vi.fn(() => []),
      shouldCancel: createStackTargetedShouldCancel(250),
      shouldPause: vi.fn(() => false),
      reportStatus: vi.fn(() => {}),
      getDiffForFiles: vi.fn().mockResolvedValue('diff ok'),
      appendEvent
  ...
  test('cancels after applying user guidance edit', async () => {
    ...
    const deps = {
      ...
      rollback: vi.fn(),
      consumeUserUpdates,
      shouldCancel: createStackTargetedShouldCancel(635),
      shouldPause: vi.fn(() => false),
      reportStatus,
      getDiffForFiles: vi.fn().mockResolvedValue('diff ok'),
      appendEvent
  ...
  test('cancels before starting the next step when requested', async () => {
    ...
    const deps = {
      ...
      rollback: vi.fn(),
      consumeUserUpdates: vi.fn(() => []),
      shouldCancel: createStackTargetedShouldCancel(318),
      shouldPause: vi.fn(() => false),
      reportStatus: vi.fn(),
      getDiffForFiles: vi.fn().mockResolvedValue('diff ok'),
      appendEvent
    ...
    await expect(
      autopilotFeatureRequest({
        ...
        deps: {
          ...
          rollback: vi.fn(),
          consumeUserUpdates: vi.fn(() => []),
          shouldCancel: createStackTargetedShouldCancel(231),
          shouldPause: vi.fn(() => false),
          reportStatus: vi.fn(),
          getDiffForFiles: vi.fn().mockResolvedValue('diff ok'),
          appendEvent,
