...
describe('autopilotFeatureRequest', () => {
  ...
  test('cancels during verification fix loop', async () => {
    ...
    const deps = {
      ...
      rollback: vi.fn(),
      consumeUserUpdates: vi.fn(() => []),
      shouldCancel: createStackTargetedShouldCancel(874),
      shouldPause: vi.fn(() => false),
      reportStatus: vi.fn(),
      getDiffForFiles: vi.fn().mockResolvedValue('diff ok'),
      appendEvent
  ...
  test('cancels after verification fix pause before retrying', async () => {
    ...
    const deps = {
      ...
      rollback: vi.fn(),
      consumeUserUpdates: vi.fn(() => []),
      shouldCancel: createStackTargetedShouldCancel(922),
      shouldPause: vi.fn(() => false),
      reportStatus: vi.fn(),
      getDiffForFiles: vi.fn().mockResolvedValue('diff ok'),
      appendEvent
  ...
  test('cancels while awaiting user guidance', async () => {
    ...
    const deps = {
      ...
      rollback: vi.fn(),
      consumeUserUpdates: vi.fn(() => []),
      shouldCancel: createStackTargetedShouldCancel(958),
      shouldPause: vi.fn(() => false),
      reportStatus: vi.fn(() => {}),
      getDiffForFiles: vi.fn().mockResolvedValue('diff ok'),
      appendEvent
  ...
  test('cancels after applying user guidance edit', async () => {
    ...
    const deps = {
      ...
      rollback: vi.fn(),
      consumeUserUpdates,
      shouldCancel: createStackTargetedShouldCancel(992),
      shouldPause: vi.fn(() => false),
      reportStatus,
      getDiffForFiles: vi.fn().mockResolvedValue('diff ok'),
      appendEvent
  ...
  test('cancels before starting the next step when requested', async () => {
    ...
    const deps = {
      ...
      rollback: vi.fn(),
      consumeUserUpdates: vi.fn(() => []),
      shouldCancel: createStackTargetedShouldCancel(675),
      shouldPause: vi.fn(() => false),
      reportStatus: vi.fn(),
      getDiffForFiles: vi.fn().mockResolvedValue('diff ok'),
      appendEvent
    ...
    await expect(
      autopilotFeatureRequest({
        ...
        deps: {
          ...
          rollback: vi.fn(),
          consumeUserUpdates: vi.fn(() => []),
          shouldCancel: createStackTargetedShouldCancel(588),
          shouldPause: vi.fn(() => false),
          reportStatus: vi.fn(),
          getDiffForFiles: vi.fn().mockResolvedValue('diff ok'),
          appendEvent,
