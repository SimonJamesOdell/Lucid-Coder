...
const listBranchChangedPaths = async (context, { baseRef = 'main', branchRef } = {}) => {
  if (!context?.gitReady) {
    return [];
  }
...
const isCssOnlyBranchDiff = async (context, branchName) => {
  if (!context?.gitReady) {
    return false;
  }
...
const resolveProjectGitSettings = async (projectId) => {
...
const runProjectGit = (context, args, options) => {
  if (!context?.gitReady) {
...
const listGitStagedPaths = async (context) => {
...
const listGitStagedStatusMap = async (context) => {
...
const parseGitLsFilesStageBlob = (stdout = '') => {
  const first = stdout
    .split(/\r?\n/)
    .map((line) => line.trim())
    .filter(Boolean)[0];
...
const listGitStagedEntries = async (context) => {
...
const buildStagedFilesSnapshot = (existingEntries, stagedEntries) => {
...
const syncCurrentBranchStagedFilesFromGit = async (projectId, context) => {
...
const ensureGitBranchExists = async (context, branchName, baseBranch = 'main') => {
...
const checkoutGitBranch = async (context, branchName) => {
  if (!context.gitReady || !branchName) {
    return;
  }
...
export const __testing = {
  ...
  buildCommitMessage,
  normalizeCommitLimit,
  setTestModeOverride: (value) => {
    testModeOverride = value;
  },
  setGitContextOverride: (projectId, projectPath) => {
    if (!projectId) {
      return;
    }
    if (projectPath) {
